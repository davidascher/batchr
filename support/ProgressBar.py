...
class ProgressBar:
    ...
    CD_BASE = "/Applications"
    CD_PATH = os.path.join(CD_BASE, "CocoaDialog.app/Contents/MacOS/CocoaDialog")
    
    def __init__(self, title="Progress", message="", percent=0, indeterminate=False):
        """Create progress bar dialog"""
        if indeterminate:
            template = "%s progressbar --title '%s' --text '%s' --indeterminate"
            self.pipe = os.popen(template % (ProgressBar.CD_PATH, title, message), "w")
        else:
